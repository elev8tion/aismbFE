/**
 * NoCodeBackend (NCB) specific API response types.
 * NCB always wraps responses in { data: T } or { data: T[] } format.
 */
...
/** NCB create response — returns { status: "success", id: number } */
export interface NCBCreateResponse {
  status: 'success';
  id: number;
}

/** NCB update/delete response — returns { status: "success" } */
export interface NCBMutationResponse {
  status: 'success';
}

/** NCB error response */
export interface NCBErrorResponse {
  error: string;
  details?: Record<string, unknown>;
}

// ─── CRM-specific API Response Types ────────────────────────────────────────
