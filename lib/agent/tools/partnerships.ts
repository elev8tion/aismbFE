...
export async function list_partnerships(params: { phase?: string; status?: string }, cookies: string) {
...
export async function update_partnership_phase(params: { partnership_id: string; phase: string }, cookies: string) {
  ...
  const result = await ncbUpdate<Partnership>('partnerships', params.partnership_id, { current_phase: params.phase }, cookies);
  return { success: true, partnership: result };
}

export async function update_satisfaction_score(params: { partnership_id: string; score: number }, cookies: string) {
  const result = await ncbUpdate<Partnership>('partnerships', params.partnership_id, { satisfaction_score: params.score }, cookies);
  return { success: true, partnership: result };
}
